...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['file'])) {
        ...
        if (empty($bus_name) || empty($price) || empty($from) || empty($to) || empty($date) || empty($bus_type)) {
            handleResponse(400, 'Invalid input. Please provide all required details.');
        }

        // Call the busimg method
        $obj = new Post();
        $result = $obj->busimg($bus_name, $from, $to, $date, $bus_type, $price, $uploadPath);
